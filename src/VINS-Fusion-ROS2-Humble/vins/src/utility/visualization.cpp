...
void pubTF(const Estimator &estimator, const std_msgs::msg::Header &header)
{
    ...
    // Publish world -> body TF
    Vector3d correct_t = estimator.Ps[WINDOW_SIZE];
    Quaterniond correct_q = Quaterniond(estimator.Rs[WINDOW_SIZE]);
    ...
    // Publish body -> camera TF
    geometry_msgs::msg::TransformStamped transform_cam;
